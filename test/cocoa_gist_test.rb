...
describe 'CocoaGist' do
  it "should lazy load the github user info once and return it as a hash" do
    ...
    CocoaGist.expects(:`).with('git config --global github.user').returns('alloy').times(1)
    CocoaGist.expects(:`).with('git config --global github.token').returns('secret').times(1)
